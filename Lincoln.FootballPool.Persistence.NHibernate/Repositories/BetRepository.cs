...
namespace Lincoln.FootballPool.Persistence.NHibernateFramework.Repositories
{
    ...
    public class BetRepository : Repository<Bet>, IBetRepository
    {
        ...
        /// <summary>
        /// Gets all a paged result set of all bets placed by the pool user with the supplied <paramref name="poolUserId"/> from the persistence store.
        /// </summary>
        ...
        public PaginatedList<Bet, int> GetBetsForWeek(int weekNumber, int poolId, PagingInfo pagingInfo)
        {
            ...
            return base.GetPaginatedList(pagingInfo, bet => bet.Game.WeekNumber == weekNumber && bet.PlacedBy.Pool.Id == poolId, bet => bet.PlacedBy.UserName);
        }
        ...
        private static Expression<Func<Bet, string>> GetSortExpression(string sortField)
        {
            BetSortField betSortField;

            ////If sort field is not valid name, throw expception.
            if (!Enum.TryParse(sortField, true, out betSortField))
            {
                throw new InvalidSortExpressionException(string.Format("{0} is not a supported field when sorting bets.", sortField));
            }
